...
//   default:        "Interest signal" — notifies recruiter without revealing candidate email
//   introduction:   "Forward introduction" — reveals candidate email to recruiter after admin approval
...
module.exports = async function handler(req, res) {
  ...
  const { data: job, error: jobErr } = await admin
    .from('fed_jobs')
    .select('id, title, firm_name, firm_email, industry, location')
    .eq('id', job_id)
    .single();
  ...
  if (isIntro) {
    // ── FORWARD INTRODUCTION — candidate email revealed to firm ──────────────
    if (firmEmail) {
      await sendZapier({
        ...
        body: `We are pleased to facilitate a formal introduction for your search — ${job.title}.\n\n` +
              ...
              `This candidate expressed interest in your posting and Fredheim has reviewed and approved this introduction. ` +
              `All further communication is directly between your firm and the candidate. ` +
              `Please reply to the candidate directly — do not reply to this email.\n\n` +
    ...
    // ── INTEREST SIGNAL — candidate identity withheld ────────────────────────
    if (firmEmail) {
      await sendZapier({
        type:          'new_interest_signal',
        to_email:      firmEmail,
