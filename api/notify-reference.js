...
module.exports = async function handler(req, res) {
  ...
  const questionnaireUrl = `https://trovanttalent.com?ref=${encodeURIComponent(token)}`;
  const subject = `${candidate_name || 'A colleague'} listed you as a reference — 5-minute questionnaire`;
  const body = `Hi ${ref_name || 'there'},\n\n${candidate_name || 'A colleague'} has listed you as a professional reference on Trovant Talent, a confidential talent marketplace.\n\nThey've asked if you'd complete a brief 5-minute questionnaire about your professional experience with them. No login is required.\n\nComplete the questionnaire here:\n${questionnaireUrl}\n\nThis link is unique to you. If you have any questions, contact contact@trovanttalent.com.\n\nTrovant Talent\ncontact@trovanttalent.com`;
