...
export default async function handler(req, res) {
  ...
  switch (event.type) {
    ...
    case 'checkout.session.completed': {
      ...
      // price_1TP6CIJ1BNVQzyrWSXVQxNNh = Executive $199/yr (live)
      // Add Senior Professional price ID here when created
      const tier = priceId === process.env.ACTIVE_PRICE_EXECUTIVE
        ? 'active'
        : priceId === process.env.ACTIVE_PRICE_SENIOR
        ? 'active_senior'
        : 'active'; // default to executive if only one price exists
